...
#define ADS1256_LOG_PRINT(CAPTION, VALUE)  \
  {                                        \
    ADS1256_Log_Print((CAPTION), (VALUE)); \
  }

#define ADS1256_LOG_DUMPREGISTERS(CAPTION) \
  {                                        \
    ADS1256_Log_DumpRegisters((#CAPTION)); \
  }
...
// Замечу, конкретно в этой функции: Если сигнал готовности идет по Внешнему Порту, то SPI интерфейс включать не нужно!
  ...
  // Синхронная задержка до готовности АЦП
  while (ADS1256_DRDY_BUSY())
    ; // (**) Замечу: при активном DATAC-режиме, и при отсутствии отдельной шины DRDY - здесь будет глюк (синхронного ожидания не будет), зацикливания и зависания программы не произойдет (что лучше), но и "готовности АЦП" не обеспечит.
